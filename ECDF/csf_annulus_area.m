...
fprintf('4. Calculating Area Distortion Metrics...\n');

cell_areas = [];
[rows, cols] = size(X_mesh);

for r = 1:rows-1
    for c = 1:cols-1
        xv = [X_mesh(r,c), X_mesh(r,c+1), X_mesh(r+1,c+1), X_mesh(r+1,c)];
        yv = [Y_mesh(r,c), Y_mesh(r,c+1), Y_mesh(r+1,c+1), Y_mesh(r+1,c)];
        area_val = polyarea(xv, yv);
        cell_areas = [cell_areas; area_val];
    end
end

mean_area = mean(cell_areas);
ratio_qcm = cell_areas / mean_area;

fprintf('   Max Area Ratio: %.4f\n', max(ratio_qcm));
fprintf('   Min Area Ratio: %.4f\n', min(ratio_qcm));

outputFile = 'Annulus_mapping.mat';
fprintf('5. Saving data to %s...\n', outputFile);
save(outputFile, 'X_mesh', 'Y_mesh', 'ratio_qcm');
fprintf('Done! You can now run "plot_comparison_type3.m".\n');
